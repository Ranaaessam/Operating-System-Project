...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(physical_address_pg_tbl, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 *Ptr_Page_Table;
	uint32 Free_Pages; // Used in Step 3
	...
	for(int i = 0; i < size; i =i + PAGE_SIZE)
	{
		...
		{
			if(e->ptr_pageWorkingSet[j].virtual_address == va)
			{
	...
	// Loop on the Tables to decide which tables should remove
	va = ROUNDDOWN(virtual_address,PAGE_SIZE*1024);
	for(int i = va; i <= (virtual_address+size); i= i+(PAGE_SIZE*1024))
	{
