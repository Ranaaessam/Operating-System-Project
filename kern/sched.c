...
struct Env* fos_scheduler_MLFQ()
{
	...
	if (curenv != NULL)
		{
			enqueue(&env_ready_queues[new_set_queue], curenv);
		}

	for (uint32 i = 0; i < num_of_ready_queues; i++)
		{
			uint32 oursize = queue_size(&env_ready_queues[i]);
			if (oursize > 0)
			{
				struct Env* new_env = dequeue(&env_ready_queues[i]);
				kclock_set_quantum(quantums[i]);
				if (i < num_of_ready_queues - 1)
					new_set_queue = i + 1;
				else
					new_set_queue = num_of_ready_queues - 1;
				return new_env;
			}
		}
	//Steps:
