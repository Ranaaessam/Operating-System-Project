...
void placement(struct Env * curenv, uint32 fault_va, uint32 replacement_index,uint32 replacement)
{
	...
	else
	{
		...
		if (ret != E_NO_MEM)
		{
			page_perm |= (PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
			ret = map_frame(curenv->env_page_directory, new_frame_info,(void*) fault_va, page_perm);
			if (ret != E_NO_MEM)
			{
				//this read page data from hard disk to memory
	...
	if (replacement == 0)
	{
		...
		uint32 WS_Size = curenv->page_WS_max_size;
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	while (true)
	{
		...
		{
			i = (i % curenv->page_WS_max_size);
			uint32 tmp_VA = env_page_ws_get_virtual_address(curenv, i);
			uint32 page_perm = pt_get_page_permissions(curenv, tmp_VA);
			if((page_perm & PERM_MODIFIED) == 0 && (page_perm & PERM_USED) == 0)
			{
				victim_VA = tmp_VA;
				victim_index = i;
				curenv->page_last_WS_index = ((i + 1) % curenv->page_WS_max_size);
		...
		i = start_index;
		WS_Size = curenv->page_WS_max_size;
		...
		while (WS_Size > 0)
		{
			...
			pt_set_page_permissions(curenv, tmp_VA, 0, PERM_USED);
			WS_Size--;
			i++;
		}
	}
	...
	uint32 page_perm = pt_get_page_permissions(curenv, victim_VA);
	if (page_perm & PERM_MODIFIED)
	{
